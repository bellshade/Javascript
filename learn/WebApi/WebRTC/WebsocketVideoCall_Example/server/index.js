...
webSocketServer.on("listening", () => console.log("WS listening to port " + PORT));
...
const packetHandler = {
    "call": ({
        ...
        if (webSocketServer.userList[id]) {
            webSocketServer.userList[id].send(JSON.stringify({
                type: "call",
                id: socket.id,
                sdp
            }))
    ...
    "newIce": (msg, socket) => {
        webSocketServer.userList[msg.id].send(JSON.stringify({
            type: "newIce",
            id: socket.id,
...
webSocketServer.on("connection", (socket) => {
    ...
    socket.on("message", (data) => { //KALO ADA MESSAGE
